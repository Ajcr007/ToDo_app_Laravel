@extends('layouts.todo.todoPage')
@section('content')
    @include('todos.subview.create')

    <div class="container py-5">
        <h3 class="text-center fw-bold"> CRUD Operations in Laravel 10 </h3>
            ...
            <table class="table table-striped" id="todo-table">
                ...
                <tbody>
                    @foreach ($todos as $todo)
                        <tr id="{{ 'todo_' . $todo->id }}">
                            <td><input type="checkbox" class="form-check-input todo-checkbox" value="{{ $todo->id }}" />
                            </td>
                            <td>{{ $todo->id }}</td>
                            <td>{{ $todo->title }}</td>
